I later cofounded a music technology startup while I was still in college that build off of this project, using much more advanced algorithms and robust data engineering practices. You can find an article about the project written by Popular Mechanics [here](https://www.popularmechanics.com/culture/music/a28965296/spotify-predicting-hit-songs/)! Thanks for stopping by!

# Summary
An example fullstack (frontend + backend) app that finds great undiscovered new music! The app demonstrates how to build a fully functioning, scalable, and cost-effective web app / product. The app is set up for local development and can be easily deployed to the Cloud. I use open source tools like Docker (including Docker Stack), Python, Postgres, Poetry, FastAPI, Celery, Redis, PyTorch, Node.js, VueJS, and TypeScript. The backend service demonstrates how to build a fully managed distributed task orchestration system with pub/sub architecture for data processing and an API for triggering tasks or fetching user requested data. The backend service is capable of processing and analyzing 100+ million data points daily and handling 100+ thousand API requests daily. The frontend service demonstrates how to build and deploy a component-based reactive web app that enables user account management, fetching and displaying backend data, and a basic CRM. Similar to the backend service, the frontend can be scaled to handle millions of daily active users. 

> Note: This example application and it's contents are meant for non-commercial demonatration purposes only. The app uses Spotify data. Please, see Spotify's [user](https://www.spotify.com/us/legal/end-user-agreement/) and [developer](https://developer.spotify.com/terms/) agreements if you plan on using any of the data retrieved by this example app.

> Note: I built this app in 2019 during my third year of undergrad for fun to learn and practice data/ML engineering and science. I attempted to follow all the best practices and use the latest technologies available for the project at the time. However, since I haven't updated the app since 2019, there is a good chance that today's best practices and technologies are different. It's also likely that my coding style and skills have changed significantly (hopefully for the better ;)) since 2019. So, I wouldn't recommend interpreting this project as the only or best solution to building a fullstack scalable web app/product, rather it can be seen as one way to solve the problem. Hopefully this project can help and inspire you to build something awesome!!!
...
In that case, you will need to use a fake local domain (`dev.musicai.io`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.musicai.io` was configured to be allowed. If you want a custom one, you need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

* Open your `hosts` file with administrative privileges using a text editor:
  * **Note for Windows**: If you are in Windows, open the main Windows menu, search for "notepad", right click on it, and select the option "open as Administrator" or similar. Then click the "File" menu, "Open file", go to the directory `c:\Windows\System32\Drivers\etc\`, select the option to show "All files" instead of only "Text (.txt) files", and open the `hosts` file.
  * **Note for Mac and Linux**: Your `hosts` file is probably located at `/etc/hosts`, you can edit it in a terminal running `sudo nano /etc/hosts`.

* Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character, and your fake local domain: `dev.musicai.io`.

The new line might look like:

```
192.168.99.100    dev.musicai.io
```

* Save the file.
  * **Note for Windows**: Make sure you save the file as "All files", without an extension of `.txt`. By default, Windows tries to add the extension. Make sure the file is saved as is, without extension.

...that will make your computer think that the fake local domain is served by that custom IP, and when you open that URL in your browser, it will talk directly to your locally running server when it is asked to go to `dev.musicai.io` and think that it is a remote server while it is actually running in your computer.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.musicai.io`.

After performing those steps you should be able to open: http://dev.musicai.io and it will be server by your stack in `localhost`.

Check all the corresponding available URLs in the section at the end.

### Change the development "domain"

If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or using `dev.musicai.io`).
...
```bash
STACK_NAME=stag-musicai-io sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.musicai-io.app-db-data == true
```
...
```bash
docker node update --label-add musicai-io.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stag-musicai-io.app-db-data=true cat.example.com
```
...
Frontend: https://musicai.io

Backend: https://musicai.io/api/

Automatic Interactive Docs (Swagger UI): https://musicai.io/docs

Automatic Alternative Docs (ReDoc): https://musicai.io/redoc

PGAdmin: https://pgadmin.musicai.io

Flower: https://flower.musicai.io

### Staging URLs

Staging URLs, from the branch `master`.

Frontend: https://stag.musicai.io

Backend: https://stag.musicai.io/api/

Automatic Interactive Docs (Swagger UI): https://stag.musicai.io/docs

Automatic Alternative Docs (ReDoc): https://stag.musicai.io/redoc

PGAdmin: https://pgadmin.stag.musicai.io

Flower: https://flower.stag.musicai.io
...
Frontend: http://dev.musicai.io

Backend: http://dev.musicai.io/api/

Automatic Interactive Docs (Swagger UI): https://dev.musicai.io/docs

Automatic Alternative Docs (ReDoc): https://dev.musicai.io/redoc

PGAdmin: http://dev.musicai.io:5050

Flower: http://dev.musicai.io:5555

Traefik UI: http://dev.musicai.io:8090
